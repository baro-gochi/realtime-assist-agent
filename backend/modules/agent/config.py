...
logger.info(f"[에이전트] 설정 로드: .env={_env_path} (존재: {_env_path.exists()})")
logger.info(f"[에이전트] LLM 모델: {llm_config.MODEL}")
logger.info(f"[에이전트] 온도: {llm_config.TEMPERATURE}")
logger.info(f"[에이전트] 최대 토큰: {llm_config.MAX_TOKENS}")
logger.info(f"[에이전트] 추론 노력: {llm_config.REASONING_EFFORT}")
logger.info(f"[에이전트] 요약 LLM 모델: {summary_llm_config.MODEL}")
logger.info(f"[에이전트] 요약 온도: {summary_llm_config.TEMPERATURE}")
logger.info(f"[에이전트] Redis 캐시 활성화: {redis_cache_config.CACHE_ENABLED}")
logger.info(f"[에이전트] Redis 캐시 타입: {redis_cache_config.CACHE_TYPE}")
